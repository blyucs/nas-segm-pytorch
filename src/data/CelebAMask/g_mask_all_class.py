...
folder_anno = os.path.join(folder_base,'CelebAMask-HQ-mask-anno')
folder_save = os.path.join(folder_base,'CelebAMask-HQ-mask-all-class')
img_num = 30000
folder_raw = os.path.join(folder_base,'CelebA-HQ-img')
raw_save = os.path.join(folder_base,'CelebA-HA-img-resize')
make_folder(folder_save)
# make_folder(raw_save)

image_id_list = open(os.path.join('.', 'train_all_class.lst'), 'w')
test_id_list = open(os.path.join('.', 'test_all_class.lst'), 'w')

for k in range(img_num):
	folder_num = k // 2000
	im_base = np.zeros((512, 512))
	for idx, label in enumerate(label_list):
		filename = os.path.join(folder_anno, str(folder_num), str(k).rjust(5, '0') + '_' + label + '.png')
